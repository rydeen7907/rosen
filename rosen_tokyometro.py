...
注1)：東京メトロ丸ノ内線 中野坂上＜＝＞方南町はデータなし
注2)：都営地下鉄と他社線への乗入部分は対象外
注3)：複数の会社線には非対応 (応急措置：都営地下鉄も含める場合は全国編で対応)
注4)：路線を多く構える会社線単独であれば このコードを改良しても行けるかも…
...
#例：全国の駅から東京メトロの駅のみ抽出する 東京メトロ...company_cd == 18
...
#例：東京メトロの接続辺を抽出する 路線...line_cd == 28001---28010
...
# 駅名をテキストファイルに書き込み
with open('route_tokyometro.txt', 'w', encoding='utf-8') as f:
